  ...
  /*
      test('sends flow', async () => {
        const result = await bot.sendFlow(
            ...
            'Get Started',
            {
              flow_id: "1060698311899382",
              flow_token: "flows-builder-5fda639e",
              screen: "LinkWhatsApp",
              footerText: 'Random footer text',
              header: {
                type: 'text',
...
describe('server functions', () => {
      ...
      },
      {
        ...
        interactive: {
          ...
          nfm_reply: {
            ...
            body: 'Sent',
            response_json: {
              flow_token: 'flows-builder-a0697cdc',
              data: {
                airtime: {
                  amount: '10',
                  currency: 'USD',
    ...
    bot.on('message', async (raw) => {
      ...
      switch (message.type) {
        ...
        case 'nfm_reply':
          ...
          expect(data).toHaveProperty('response_json');
          expect(typeof data.name).toBe('string');
          expect(typeof data.body).toBe('string');
          expect(typeof data.response_json === 'object').toBe(true);
          expect(data.response_json).toHaveProperty('flow_token');
          expect(data.response_json).toHaveProperty('data');
          expect(data.response_json).toHaveProperty('screen');
          expect(typeof data.response_json.flow_token).toBe('string');
          expect(typeof data.response_json.data).toBe('object');
          expect(typeof data.response_json.screen).toBe('string');
          break;
        default:
          break;
      }
  ...
  test('listen for group lifecycle webhook', (): Promise<void> => new Promise((resolve, reject) => {
  ...
  test('listen for calls webhook', (): Promise<void> => new Promise((resolve, reject) => {
